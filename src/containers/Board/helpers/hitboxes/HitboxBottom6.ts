...
class HitboxBottom6 extends Hitbox {
    constructor(
        textureName: string,
    ...
    update(deltaTime: number, table: Table): void {
        ...
        this._sprite.scale.set((scaleFactor * 0.117) / 2, scaleFactor * 0.029);
        const spriteBounds = this._sprite.getBounds();
        const offsetX = this._offset.x * spriteBounds.width * 1.42;
        const offsetY = this._offset.y * spriteBounds.height * 1.46;
        this._sprite.x = offsetX + (tableRect.x + tableRect.width / 1.2);
        this._sprite.y = tableRect.y + tableRect.height * 0.718 - offsetY;
    }
}

export default HitboxBottom6;
